        ...
        for pix_i in range(0, len(img_array[0])):
            ...
            for j in range(-effective_area, effective_area):
                for i in range(-effective_area, effective_area):
                    ...
                    brightness_pix = (int(img_array[clamped_pix_j, clamped_pix_i]) - 127) / 128
...
if __name__ == "__main__":
    if image_path.suffix == ".gif":
        ...
        converted_image.save(f'{sys.argv[1]} bl_size:{block_size} eff_area:{effective_area} stren_mul:{strength_multiplier}.png')
